...
| M5 | Content: the real question bank — deep (8+ per subtopic, 200+ questions), authored by David from his study materials, difficulty-tagged 1–3 — plus data-layer hardening for authoring at scale, the `subject` field (settled, see §7), per-topic file split, and the Topics screen learning to scroll (list scrolls, Start button stationary). *Scope settled 2026-08-31, see `milestones/m5.md`* | 10–16 |
