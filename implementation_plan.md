...
- **Randomized question rotation within a topic.** M3 hands out questions in deterministic load order — testable, but memorizable. The forcing function is Phase 3's planned topic selection: when the user picks which topics are on the board, a game might run on only 1–3 of them, and a fixed rotation becomes obvious fast. Switch `next_question` to a shuffled draw (per-topic shuffle, reshuffle on recycle); same test strategy as randomized flee — seed it or assert membership.
...
- **Blue's square holds a random topic.** Planned evolution of the question-gated catch (§4): while Blue stands on a square, that square temporarily *possesses a random topic* — the catch question could be anything — reverting to the square's assigned topic when Blue moves off. Makes the final catch a test of everything rather than one known category. Build the possession/reversion logic during playtesting once the gated catch is proven in its simple form.
